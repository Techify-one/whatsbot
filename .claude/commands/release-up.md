...
5. Atualize o arquivo `WHATSBOT_VERSION` (JSON na raiz, lido por `/api/update` no self-update do painel):
   ...
   - Zere o campo `"popup_shown"` de nível superior para `false` — é a flag que controla se o popup de novidades já apareceu para essa instalação; toda versão nova nasce "não mostrada"
   ...
   ```
   Esse arquivo viaja dentro do zip da tag — é ele que o botão "Atualizar" do painel usa para saber a versão instalada e mostrar o popup de novidades após o usuário atualizar. **Sempre bumpe junto com a tag**, senão o self-update para de refletir a versão real. **Não esqueça o `popup_shown: false`** — esquecer deixa o arquivo com o valor `true` da release anterior (já commitado), e o popup nunca aparece para quem atualizar para essa versão.
